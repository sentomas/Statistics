...
<style>
    ...
    div[data-testid="stMetricValue"] { font-size: 1.3rem; color: #2980b9; }
    div.stButton > button { 
...
# --- 3. SESSION STATE ---
state_vars = ['meta_data', 'df_case', 'df_ctrl', 'gen_results', 'clin_results', 'meta_img_buf', 'heatmap_img_buf', 'bias_stats', 'het_stats']
for var in state_vars:
    if var not in st.session_state: st.session_state[var] = None
...
def create_word_report(investigator, study_id, date):
    ...
    if st.session_state['clin_results'] is not None:
        doc.add_heading('2. Clinical Variable Analysis', level=1)
        df_clin = st.session_state['clin_results']
        table = doc.add_table(rows=1, cols=len(df_clin.columns))
        table.style = 'Table Grid'
        for i, col in enumerate(df_clin.columns): table.rows[0].cells[i].text = str(col)
        for index, row in df_clin.iterrows():
            row_cells = table.add_row().cells
            for i, val in enumerate(row): row_cells[i].text = str(val)
        doc.add_paragraph("\n")

    # 3. Meta-Analysis (Updated with Heterogeneity)
    if st.session_state['meta_img_buf'] is not None:
        doc.add_heading('3. Meta-Analysis: Bias & Heterogeneity', level=1)
        
        # Heterogeneity
        if st.session_state['het_stats']:
            het = st.session_state['het_stats']
            doc.add_heading('Heterogeneity Statistics', level=2)
            doc.add_paragraph(f"Cochran's Q: {het['Q']}")
            doc.add_paragraph(f"P-value (Q): {het['p_Q']}")
            doc.add_paragraph(f"I-squared (I²): {het['I2']}")
            doc.add_paragraph(f"Tau-squared (τ²): {het['tau2']}")

        # Bias
        if st.session_state['bias_stats']:
            bias = st.session_state['bias_stats']
            doc.add_heading('Publication Bias', level=2)
            doc.add_paragraph(f"Egger's Test P: {bias['egger']}")
            doc.add_paragraph(f"Begg's Test P: {bias['begg']}")
...
def clean_and_normalize(df):
    ...
    for col in df.columns:
        if col not in ['Name', 'Marital Status']: df[col] = pd.to_numeric(df[col], errors='coerce')
    return df
...
with st.sidebar:
    ...
    if st.button("📄 Generate Report"):
        if any(x is not None for x in [st.session_state['gen_results'], st.session_state['clin_results'], st.session_state['heatmap_img_buf']]):
            ...
            st.download_button("📥 Download Full Report (.docx)", docx_file.getvalue(), f"{study_id}_Report.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
...
if file_case and file_ctrl:
    ...
    except Exception as e: st.sidebar.error(f"Error loading files: {e}")
...
# --- 7. MAIN INTERFACE ---
st.title(f"🧬 GenEpi Analytics: {study_id}")
st.markdown(f"**Generated for:** {investigator} | **Date:** {report_date}")

tabs = st.tabs(["🧬 Genotype Analysis", "🏥 Clinical Stats", "📚 Meta-Analysis", "🎨 Heatmap Analysis"])

# ================= TAB 1: GENOTYPE =================
with tabs[0]:
    st.subheader("1. Genotypic Association")
    with st.expander("📝 Data Entry", expanded=True):
    ...
    if st.button("🚀 Run Genetic Analysis"):
        results = []
...
# ================= TAB 3: META-ANALYSIS =================
with tabs[2]:
    st.subheader("3. Meta-Analysis: Bias & Heterogeneity")
    ...
    with c_dat:
        ...
        if st.button("Run Meta-Analysis"):
            try:
                ...
                # Fixed Effect Model weighted mean
                sum_w = df['Weights'].sum()
                sum_w_logor = (df['Weights'] * df['LogOR']).sum()
                pooled_logor = sum_w_logor / sum_w
                
                # Cochran's Q
                # Q = sum(w * (y - y_pooled)^2)
                Q = (df['Weights'] * (df['LogOR'] - pooled_logor)**2).sum()
                df_Q = len(df) - 1
                ...
                C_const = sum_w - (df['Weights']**2).sum() / sum_w
                tau2 = max(0, (Q - df_Q) / C_const) if C_const > 0 else 0
                
                st.session_state['het_stats'] = {
                    'Q': f"{Q:.2f}", 'p_Q': f"{p_Q:.4f}",
                    'I2': f"{I2:.1f}%", 'tau2': f"{tau2:.4f}"
                }
                ...
                with col_h1:
                    st.markdown("#### Heterogeneity")
                    st.metric("Cochran's Q", f"{Q:.2f}")
                    st.metric("I-squared (I²)", f"{I2:.1f}%")
                    st.metric("P-value (Heterogeneity)", f"{p_Q:.4f}")
                with col_h2:
                    st.markdown("#### Publication Bias")
                    st.metric("Egger's P", f"{egg_p:.4f}")
                    st.metric("Begg's P", f"{begg_p:.4f}")
                ...
                with c_plot:
                    ...
                    ax.axvline(pooled_logor, color='red', linestyle='--', label="Pooled OR")
                    st.pyplot(fig)
